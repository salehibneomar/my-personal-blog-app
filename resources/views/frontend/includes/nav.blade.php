        ...
        <header class="group top-nav">
            <nav class="navbar logo-w navbar-left" >
                <a class="logo" href="{{ route('index') }}" title="{{ $site_info->settings->name }}">
                    <img src="{{ $site_info->settings->logo }}" alt="{{ $site_info->settings->name }}" title="{{ $site_info->settings->name }}">&ensp;
                    {{ $site_info->settings->name }}
                </a>
            </nav>
            <div class="navigation-toggle" data-tools="navigation-toggle" data-target="#navbar-1">
                <span class="logo" title="{{ $site_info->settings->name }}" >
                    <img src="{{ $site_info->settings->logo }}" alt="{{ $site_info->settings->name }}" title="{{ $site_info->settings->name }}" >&ensp;{{ $site_info->settings->initial_name }}
                </span>
            </div>
